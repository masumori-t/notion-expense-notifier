...
Notionの「小口精算フォーム」回答データベースを定期的にチェックし、「総務清算完了」になっていないデータを回答者ごとに集計します。
**まだタスク通知していない未清算データがある場合だけ**、Notionの担当者(あなた)に「小口清算をする」タスクを自動で作成・アサインします。未通知が無ければ何もしません(タスクは重複作成されません)。
...
[notify.js] ─② 未通知のpageIdがあるか判定(state/last-checked.json)
        ...
        ├─ 未通知なし → 何もしない(終了)
        │
        └─ 未通知あり ─③ 回答者ごとに集計し、Notionページ本文(チェックリスト)を組み立てる
                              ▼
                    [Notion: KT_タスクDB]
                    「💴 小口清算をする」タスクを作成
                    ・対応者 = あなた
                    ・本文 = 現在未清算の全データ(🆕マーク付きで未通知分を明示)
```
...
- 未清算の各回答ページIDを、状態ファイル内の `notifiedPageIds`(すでにタスク通知したID一覧)と比較する。
- 一覧に無いページIDが1件でもあれば「未通知あり」と判定し、タスクを作成する。
- タスク作成に成功した場合のみ `state/last-checked.json` を更新し、GitHub Actionsがそれをリポジトリにコミットする。
  (失敗時は更新しないため、次回実行で同じデータを再検知できる)
- 旧形式(時刻のみ)の状態ファイルからは、時刻比較にフォールバックして移行する。
...
- **上司の承認**: 実データベースには承認用の実質的なプロパティが存在しない(タイトル列が偶然この名前になっているだけ)ため、承認フィルタは使わず「総務清算完了」のみで未清算を判定する
